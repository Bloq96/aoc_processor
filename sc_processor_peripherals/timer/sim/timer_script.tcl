...
if {[file isdirectory sim]} {
	...
	vcom -2008 -work work ../../rtl/timer.vhd

	vcom -2008 -work work ../testbench_timer.vhd

	vsim -t 1ps -L altera -L lpm -L sgate -L altera_mf -L altera_lnsim -L cycloneii -L rtl_work -L work -voptargs="+acc"  testbench_timer
	vsim -L altera -L lpm -L sgate -L altera_mf -L altera_lnsim -L cycloneii -L rtl_work -L work -voptargs=\"+acc\" -t 1ps testbench_timer

	add wave *
	add wave /T/w_count
	add wave /T/DIVx/w_max_count
	add wave /T/CNTx/w_max_count
	view structure
	view signals
	run 10 us
}
